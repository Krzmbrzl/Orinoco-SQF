...
public interface OrinocoLexerStream extends OrinocoTokenProcessor {
	 ...
	 * {@link #preProcessorTokenSkipped(String, int)} will be invoked for each
	 * whitespace separated "word"/token that wasn't preprocessed. Also, each
	 * command will be sent to {@link #preProcessorCommandSkipped(String, int)}
	 *
	...
	/**
	 * Accepts a whole block of text that contains the whole preprocessor command
	 * and the body of the command (if applicable). Here is the format:
	 * <b>#commandName body?</b>. The ? means optional.
	 *
	 * @param command the command matched
	 * @param bufReadOnly the read-only char[] buffer that contains the whole
	 *        command and body
	 * @param offset the offset in the buffer where the command begins, starting
	 *        with #
	 ...
	 *
	 * @param originalOffset text offset at which the whitespace begins in the
	 *        original input
	 * @param originalLength length of the whitespace area in the original input
	 * @param preprocessedOffset text offset at which the whitespace begins in the
	 *        preprocessed input. <b>This offset should be used when emitting
	 *        (error) messages.</b>
	 * @param preprocessedLength length of the whitespace area in the preprocessed
	 *        input. If the whitespace was not inserted in the input by
	 *        preprocessing, this length is equal to <code>originalLenght</code>.
	 ...
	 *        whitespace. <b>This length should be used when emitting (error)
	 *        messages.</b>
	 */
	 ...
	 * @param preprocessedOffset text offset at which the comment begins in the
	 *        preprocessed input. <b>This offset should be used when emitting
	 *        (error) messages.</b>
	 * @param preprocessedLength length of the commentin the preprocessed input. If
	 *        the comment was not inserted in the input by preprocessing, this
	 *        length is equal to <code>originalLenght</code>. However if it was
	 *        introduced by preprocessing, this length will be the length of the
	 *        preprocessor-token that was expanded to insert this whitespace.
	 *        <b>This length should be used when emitting (error) messages.</b><br>
	 *        Note that a comment can only be inserted by preprocessing if the
	 *        preprocessor is instructed to keep comments.
	 */
