...
class SQFCommandSyntaxXMLLoader {
	...
	/**
	 * Gets a {@link ValueType} from an Element's attribute. This can either return a {@link BaseType} or a {@link CodeType} if the element
	 * has a "code-handler" child tag.
	 *
	 * @param ele element that contains a type
	 * @param attributeNameWithType the attribute name that to get a type from
	 * @return the {@link BaseType#valueOf(String)} of the attribute, or {@link BaseType#_ERROR} if the type was invalid. If the {@link
	 * BaseType#valueOf(String)} returns a {@link BaseType#CODE} and a "code-handler" tag is present, a {@link CodeType} will be returned.
	 * Note that if the type in the "code-handler" can't be determined, it will have a {@link BaseType#_ERROR}
	 */
	@NotNull
